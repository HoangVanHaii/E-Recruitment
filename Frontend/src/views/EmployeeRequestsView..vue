<template>
  ...
  // Biến lưu trạng thái đang được chọn
  const selectedStatus = ref<string>('all');
  
  // Cấu hình 4 div bộ lọc
  const filterTabs = [
  ...
  // Hàm gán trạng thái mới và gọi lại API
  ...
  // Các hàm tiện ích xử lý giao diện
  ...
  const handleAction = async (id: number, action: 'Approved' | 'Rejected') => {
    ...
    if (useEmployer.error) {
        ...
        // Load lại dữ liệu theo tab hiện tại
